...
contract BarTips is Ownable {
    ...
    function emptyTheTipsJar() public payable{
        ...
        uint _tipAmountPerBartender = address(this).balance /_bartendersQuantity;
        uint counter = 0;
    for (uint i = 0; i < _bartendersQuantity; i++) {
                address payable to == bartenders[i];

          (payable(bartenders(Bartender))[i]).transfer(_tipAmountPerBartender);
          counter++;
      }
    }
